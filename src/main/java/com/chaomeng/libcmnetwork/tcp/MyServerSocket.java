...
public class MyServerSocket extends Emitter {
        ...
        new Thread(()->{
            try{
                ...
                while(true){
                    ...
                    destory();
                    new MyServerSocket.HandlerThread(client);
                    mSLog.d(TAG,"end");
                }
            }catch (Exception e){
                e.printStackTrace();
    ...
    private class HandlerThread implements Runnable {
        ...
        @Override
        public void run() {
            if (MyServerSocket.this.connected) return;
            try{
...
//            close(e.getMessage());
    ...
    private void startHeartbeat() {
        Observable.interval(mOptions.heartbeatInterval, TimeUnit.MILLISECONDS).takeWhile((w)->{return connected;
            ...
            MyServerSocket.this.send(SocketEvent.PING);
        });
    }
    ...
    public Emitter emit(final String event, boolean isinner , final Object... args) {
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                ...
                if (MyServerSocket.this.connected) {
                    //放在发送消息队列
                    MyServerSocket.this.packet(packet);
//                    waitFeedBack(packet);
                } else {
                    //放在离线送消息队列
                    MyServerSocket.this.sendBuffer.add(packet);
                }
            }
        });
        return this;
                ...
                if (event!=null){
                    if (event.equals(SocketEvent.PING)||event.equals(SocketEvent.PONG)){
                        if (event.equals(SocketEvent.PING)){
                            MyServerSocket.this.send(SocketEvent.PONG,false);
    ...
    public boolean isConnected(){
        return MyServerSocket.this.connected;
    }
    /**
     *  重连完成
     */
    ...
    private void packet(Packet packet) {
        final MyServerSocket self = this;
        ...
        if (!self.sending) {
            ...
            try{
                ...
                if (mOptions.debug){
                    mSLog.d(TAG,"packet = "+ByteUtils.bytesToHexFun3(packet.getByte()));
                }
                mOutputStream.write(packet.getByte());
                mOutputStream.flush();
                if (mOptions.debug){
                    mSLog.d(TAG,"cost = "+(System.currentTimeMillis()-start));
                }
            }catch (Exception e){
                e.printStackTrace();
                emit(SocketEvent.ERROR,e);
